...
import { ArrowLeft, ArrowRight, Plus, X, Upload, Image as ImageIcon, Check, FileDown, RotateCcw, ChevronDown } from 'lucide-react';
...
interface ClinicalCase {
  ...
  title: string; photo: string | null; preview: string | null;
}

const blankCase = (): ClinicalCase => ({
  category: '', customCategory: '', title: '', photo: null, preview: null,
});
...
export default function CVWizard() {
  ...
  const handleDownloadPdf = () => {
    const processedCases = form.cases.map(c => ({
      category: c.category === 'custom' ? c.customCategory : (CONFIG.caseCategories.find(cat => cat.id === c.category)?.en || c.category || 'General'),
      title: c.title, photo: c.photo,
    }));
    generateCvPdf({
        ...
        <div className="wizard-step-enter">
          ...
          {currentStep === 'contact' && (
            <section className="space-y-5">
              ...
              <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 px-4 py-2.5 text-xs text-amber-700 dark:text-amber-300">
                Your WhatsApp number will be a clickable link in the PDF.
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          {/* ── PROFESSIONAL SKILLS (English only) ──────────────────────────────── */}
          {currentStep === 'skills' && (
          ...
          {currentStep === 'preview' && (
            <section className="space-y-6">
              ...
              <div className="text-xs text-center text-muted-foreground">
                The PDF will include your WhatsApp as a clickable link
              </div>

              <button onClick={() => { setForm(blankForm()); setStep(0); setReady(false); }}
